...
class S3Client:
    def __init__(self, endpoint=None):
        try:
            PUBLIC_KEY, SECRET_KEY, DEFAULT_ENDPOINT = self.get_auth_keys()
            ...
            logger.error(f"Could not connect to S3 endpoint {self.endpoint}: {e.__traceback__.tb_lineno}:{e}")
            raise
    def get_auth_keys(self):
        try:
            PUBLIC_KEY = os.getenv('S3_PUBLIC_KEY')
            SECRET_KEY = os.getenv('S3_SECRET_KEY')
            DEFAULT_ENDPOINT = os.getenv('S3_ENDPOINT')
            return PUBLIC_KEY, SECRET_KEY, DEFAULT_ENDPOINT
        except Exception as e:
            logger.error(f"Could not get auth keys: {e.__traceback__.tb_lineno}:{e}")
            raise
